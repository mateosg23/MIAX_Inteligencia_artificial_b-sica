...
def enventanar_W_out_1(series, se_saben_antes, W_in=1, serie_target=0):
    n = len(series[0])
    dataX = NAN*np.ones((n,W_in,len(series)))
    #if np.any([s.dtype == object for s in series]):
    #    dataX = dataX.astype(object)
    dataY = serie_target.copy()
    
    for i in range(n):
        for j,s in enumerate(se_saben_antes):
            int_s = int(s) # para pasar True a 1 y False a 0
            ini_X = max([0,W_in-i-int_s])
            dataX[i, ini_X:,j] = series[j][max([0,i-W_in+int_s]):min([n,i+int_s])]
